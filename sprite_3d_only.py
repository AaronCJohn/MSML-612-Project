...
CHECKPOINT = "ckpt_epoch_400.pt"             # path to .pt checkpoint
OUTPUT_DIR = "./generated"                   # where images are saved
SEED       = None                            # set an int for reproducibility

# --- Generation mode ---
# "single" : generate NUM_SAMPLES images for one type/stage combo
# "chain"  : evolution chain (base → evo1 → evo2), each image fed into next
# "random" : N_RANDOM random type/stage combinations
MODE = "single"
...
# Helpers
def make_cond_vec(type_names, stage):
    ...
    return torch.cat([type_vec, stage_vec])
...
def run_single(model, ema, noise_schedule, device):
    ema.apply_shadow()
    try:
        cond = make_cond_vec(GEN_TYPES, GEN_STAGE)
        ...
        results = []
        tag = f"{'_'.join(GEN_TYPES)}_{GEN_STAGE.replace(' ', '_')}"
...
def run_chain(model, ema, noise_schedule, device):
    ...
    try:
        ...
        for stage in ALL_STAGES:
            cond = make_cond_vec(GEN_TYPES, stage)
        ...
        fname = f"{OUTPUT_DIR}/chain_{'_'.join(GEN_TYPES)}.png"
        strip.save(fname)
...
def run_random(model, ema, noise_schedule, device):
    ...
    for i in range(N_RANDOM):
        ...
        print(f"[{i+1}/{N_RANDOM}] types={type_names}, stage={stage}")
        ema.apply_shadow()
        try:
            cond = make_cond_vec(type_names, stage)
            imgs = generate(model, noise_schedule, cond, device, None)
            stage_safe = stage.replace(" ", "_")
            for j in range(imgs.shape[0]):
                pil = tensor_to_pil(imgs[j])
                all_imgs.append(pil)
                pil.save(f"{OUTPUT_DIR}/rand_{i+1}_{'_'.join(type_names)}_{stage_safe}_{j}.png")
        finally:
            ema.restore()
