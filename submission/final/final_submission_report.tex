...
Pokemon-inspired character art must balance creativity with faithfulness 
to the overall aesthetic: dual elemental types, staged
evolution chains, and a consistent look-and-feel across the distinct
official asset styles. The core problem in this project is whether a deep 
learning model can generate novel Pokémon-like designs that follow these 
patterns while remaining distinct from any species in the training set.
...
base-to-evolution progression. The objective of this project is
...
probabilistic models (DDPMs) generate images by learning to iteratively
remove noise, providing a practical alternative to GANs for
high fidelity synthesis {[}1{]}. Follow-on work improved stability and
sample quality through better noise schedules, parameterizations, and
sampling procedures {[}2{]}. Diffusion models also support conditioning
via guidance mechanisms, which can improve adherence to inputs at
inference time {[}3{]}. For higher resolutions, latent-space diffusion
reduces compute by operating in a learned compressed representation
{[}4{]}, and alternative objectives broaden the design space of diffusion
training {[}5{]}. We use classifier-free guidance to strengthen
conditional generation at sampling time {[}6{]}.
...
primarily target standalone Pokemon-like images rather than controlled
generation. Generated-Pokemon trains an unconditional DDPM/DDIM model on
819 JPG images and does not condition on type, style, evolution stage, or
previous evolutions {[}16{]}. Wong's CS230 Pokemon generator uses
StyleGAN2-ADA and a separate name-generation component, but it does not
use a diffusion objective and it does not provide explicit evolution or
attribute controls {[}17{]}. In contrast, our contribution is a
structured, controllable, evolution-aware dataset and model intended to
support attribute- and evolution-conditioned generation.
...
We construct a dataset that integrates three complementary visual sources
and augments each entry with structured Pokemon metadata:
...
\subsection{Cross-format Mapping }\label{cross-format-mapping-interim-paired-translation-era}

Cross-format mapping required explicit source-to-target image pairs to
link the same Pokemon across visual domains. We implemented scripts
that parsed filenames, normalized naming differences, handled form-specific
cases (Mega, Gigantamax, regional variants), and exported the mappings to CSV files.
Because naming conventions were inconsistent, automatic matching alone was insufficient so
we manually verified the mapped pairs.
...
}\label{diffusion-ready-dataset-final}

For the final conditional diffusion model, we constructed a unified
generative dataset in which \textbf{every entry is a (target image,
conditioning vector) pair}, optionally with a previous-evolution image
to support evolution-chain conditioning. The per-entry schema is:
